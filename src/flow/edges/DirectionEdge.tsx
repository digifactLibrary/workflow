...
export default function DirectionEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, markerEnd, style, data }: EdgeProps) {
  ...
  const color = (data as any)?.kind === 'true' ? '#10b981' : (data as any)?.kind === 'false' ? '#ef4444' : (style as any)?.stroke
  return (
    <>
      ...
      {(data as any)?.label ? (
        <EdgeLabelRenderer>
          <div
            ...
            className={`rounded border bg-background/90 px-1.5 py-0.5 text-xs shadow ${ (data as any)?.kind === 'true' ? 'text-emerald-700 border-emerald-200' : (data as any)?.kind === 'false' ? 'text-rose-700 border-rose-200' : 'text-slate-700' }`}
          >
            {(data as any)?.label}
          </div>
        </EdgeLabelRenderer>
