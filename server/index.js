...
async function loadDiagramWithRelations(diagramId, ownerId) {
  ...
  const diagramResult = await db.query(
    'SELECT id, name, created_at as "createdAt", updated_at as "updatedAt", data, active_module as "activeModule", approval FROM section0.cr07Bdiagrams WHERE id = $1 AND owner_id = $2',
    [diagramId, ownerId]
  )
...
app.put('/api/diagrams/:id', authRequired, async (req, res) => {
  try {
    ...
    if (name !== undefined && data !== undefined) {
      const r = await db.query(
        'UPDATE section0.cr07Bdiagrams SET name=$2, data=$3, active_module=COALESCE($4, active_module), approval=COALESCE($5, approval), updated_at=now() WHERE id=$1 AND owner_id=$6 RETURNING id, name, created_at as "createdAt", updated_at as "updatedAt", active_module as "activeModule", approval',
        [id, String(name), data, activeModule ?? null, typeof approval === 'boolean' ? approval : null, req.userId]
      )
    ...
    if (name !== undefined) {
      const r = await db.query(
        'UPDATE section0.cr07Bdiagrams SET name=$2, active_module=COALESCE($3, active_module), approval=COALESCE($4, approval), updated_at=now() WHERE id=$1 AND owner_id=$5 RETURNING id, name, created_at as "createdAt", updated_at as "updatedAt", active_module as "activeModule", approval',
        [id, String(name), activeModule ?? null, typeof approval === 'boolean' ? approval : null, req.userId]
      )
      if (r.rowCount === 0) return res.status(404).json({ error: 'Not found' })
      return res.json(r.rows[0])
    }
    if (data !== undefined) {
      const r = await db.query(
        'UPDATE section0.cr07Bdiagrams SET data=$2, active_module=COALESCE($3, active_module), approval=COALESCE($4, approval), updated_at=now() WHERE id=$1 AND owner_id=$5 RETURNING id, name, created_at as "createdAt", updated_at as "updatedAt", active_module as "activeModule", approval',
        [id, data, activeModule ?? null, typeof approval === 'boolean' ? approval : null, req.userId]
      )
      if (r.rowCount === 0) return res.status(404).json({ error: 'Not found' })
