...
CREATE TABLE IF NOT EXISTS section0.cr08workflow_instances (
  ...
  started_by INT4,  -- User who initiated the workflow
...
CREATE INDEX IF NOT EXISTS node_states_workflow_instance_id_idx ON section0.cr08anode_states(workflow_instance_id);
CREATE INDEX IF NOT EXISTS node_states_status_idx ON section0.cr08anode_states(status);
CREATE INDEX IF NOT EXISTS node_inputs_node_state_id_idx ON section0.cr08bnode_inputs(node_state_id);
CREATE INDEX IF NOT EXISTS node_approvals_node_state_id_idx ON section0.cr08cnode_approvals(node_state_id);
CREATE INDEX IF NOT EXISTS node_approvals_user_id_idx ON section0.cr08cnode_approvals(user_id);